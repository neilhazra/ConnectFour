...
class Connect4Board:
    ...
    def __init__(self, whoseMove = 1, rows=6, columns=7, board = None):
        ...
        self.horizontal_indices_list = []
        self.vertical_indices_list = []
        self.diagonal_indices_list = []
        for i in range(0, self.rows):
            for j in range(0, self.columns):
                if j + 3 < self.columns:
                    self.horizontal_indices_list.append([(i, j), (i, j + 1), (i, j + 2), (i, j + 3)])
                if i + 3 < self.rows:
                    self.vertical_indices_list.append([(i, j), (i + 1, j), (i + 2, j), (i + 3, j)])
                if i + 3 < self.rows and j + 3 < self.columns:
                    self.diagonal_indices_list.append([(i, j), (i + 1, j + 1), (i + 2, j + 2), (i + 3, j + 3)])
    ...
    def score_player(self, color):
        ...
        if self.four_in_a_row == -color:
            return -1000
        score = 0
        for four_row in self.horizontal_indices_list:
            score += score_row(self.board[tuple(np.array(four_row).T)], color)
        for four_row in self.vertical_indices_list:
            score += score_row(self.board[tuple(np.array(four_row).T)], color)
        for four_row in self.diagonal_indices_list:
            score += score_row(self.board[tuple(np.array(four_row).T)], color)
        return score
